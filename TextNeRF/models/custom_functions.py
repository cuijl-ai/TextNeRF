...
class VolumeRenderer(torch.autograd.Function):
    ...
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, colors, deltas, ts, rays_a, T_threshold):
        total_samples, opacity, depth, color, ws = \
            vren.composite_train_fw(sigmas, colors, deltas, ts,
                                    rays_a, T_threshold)
        ctx.save_for_backward(sigmas, colors, deltas, ts, rays_a,
                              opacity, depth, color, ws)
        ctx.T_threshold = T_threshold
        return total_samples.sum(), opacity, depth, color, ws

    @staticmethod
    @custom_bwd
    def backward(ctx, dL_dtotal_samples, dL_dopacity, dL_ddepth, dL_dcolor, dL_dws):
        sigmas, colors, deltas, ts, rays_a, \
        opacity, depth, color, ws = ctx.saved_tensors
        dL_dsigmas, dL_dcolors = \
            vren.composite_train_bw(dL_dopacity, dL_ddepth, dL_dcolor, dL_dws,
                                    sigmas, colors, ws, deltas, ts,
                                    rays_a,
                                    opacity, depth, color,
                                    ctx.T_threshold)
        return dL_dsigmas, dL_dcolors, None, None, None, None
